...
### 公司项目
### 手机互联 安卓开发 (2024年3月到2025年1月)
项目概述：日产某款车型的车机上的手机互联内置应用

开发语言：Kotlin

#### 项目描述：
1. 使用HiCar提供的sdk连接华为手机。
2. 使用Carlink提供的sdk连接iccoa联盟支持的手机。
3. 使用系统接口管理蓝牙和热点。
4. 使用状态机管理应用的状态。

#### 项目业绩：
1. 排查并成功修复因为获取ip地址错误而导致的连接失败问题。
2. 排查并成功修复投屏黑屏问题。
3. 排查并成功修复蓝牙初始化导致的连接失败问题。
4. 重构应用减少业务逻辑错误。
5. 在车机显示HiCar和Carlink的通话弹窗。
6. 应用界面适配车机屏幕尺寸。
 
### 能源中心 安卓开发 (2024年3月到2025年1月)
项目概述：日产某款车型的车机上的能源中心内置应用
...
### Laucher和系统ui 安卓开发 (2024年10月到2025年1月)
项目概述：日产某款车型的车机上的Launcher和系统ui

开发语言: Java
#### 项目描述：
1. 通知卡片、应用列表、下拉设置、导航栏、状态栏等界面元素的显示、编辑和持久化。
2. 使用应用间通讯跟其他内置应用协调显示界面内容。

#### 项目业绩：
1. 修复卡片无法拖拽到第一个位置的问题。
2. 修复编辑下拉设置界面时某个元素无法拖动的问题。
3. 适配ui。
4. 修复业务逻辑错误。
 
### 车机应用界面适配 安卓开发 (2024年1月到2024年8月)
项目概述：东风某个车机上的内置应用界面适配

开发语言: Java

#### 项目描述：
1. 大概10来个车机内置应用需要适配界面。
